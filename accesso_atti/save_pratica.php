...
if ($resultCheck->num_rows > 0) {
    ...
    if ($result->num_rows > 0) {
        while($row = $result->fetch_assoc()) {
            ...
            $sqlDEL = "DELETE FROM attivita WHERE cf = '". $_SESSION['CF'] ."' AND servizio_id = 6 AND status_id = 1 AND pratica_id = ".$row['id_orig'];
            $connessioneDEL->query($sqlDEL);

            $sqlDEL = "DELETE FROM attivita WHERE cf = '". $_SESSION['CF'] ."' AND servizio_id = 6 AND status_id = 0 AND pratica_id = ".$_POST['pratican'];
            $connessioneDEL->query($sqlDEL);                

            $sqlDEL = "DELETE FROM accesso_atti WHERE id = ".$row['id_orig'];
            $connessioneDEL->query($sqlDEL);

            $sqlDEL = "DELETE FROM accesso_atti WHERE id = ".$_POST['pratican'];
            $connessioneDEL->query($sqlDEL);
    ...
    if($configDB['ProtocollazioneAttiva']){
        ...
        $sqlUPD = "UPDATE accesso_atti SET NumeroProtocollo='".$newProtocollo."' WHERE id = '".$new_id."'";
    ...
    if($configDB['AppIoAttiva']){
        SendToAppIo('accesso_atti',$NumeroPratica);
    }
