    ...
    if(isset($_GET["aa_pratica_id"]) && $_GET["aa_pratica_id"]<>''){
        ...
        // output data of each row
            while($row = $result->fetch_assoc()) {
                ...
                $uploadAffittuario = $row["uploadAffittuario"] != "" ? "<li class='upload-file success'><svg class='icon icon-sm' aria-hidden='true'><use href='../lib/svg/sprites.svg#it-file'></use></svg><p>". $row["uploadAffittuario"] ."</p></li>" : "";
                $uploadAltroSoggetto = $row["uploadAltroSoggetto"] != "" ? "<li class='upload-file success'><svg class='icon icon-sm' aria-hidden='true'><use href='../lib/svg/sprites.svg#it-file'></use></svg><p>". $row["uploadAltroSoggetto"] ."</p></li>" : "";
                $uploadNotaioRogante = $row["uploadNotaioRogante"] != "" ? "<li class='upload-file success'><svg class='icon icon-sm' aria-hidden='true'><use href='../lib/svg/sprites.svg#it-file'></use></svg><p>". $row["uploadNotaioRogante"] ."</p></li>" : "";
                $uploadAltriTitoloDescrizione = $row["uploadAltriTitoloDescrizione"] != "" ? "<li class='upload-file success'><svg class='icon icon-sm' aria-hidden='true'><use href='../lib/svg/sprites.svg#it-file'></use></svg><p>". $row["uploadAltriTitoloDescrizione"] ."</p></li>" : "";
                ...
                $uploadAttoNotarile = $row["uploadAttoNotarile"] != "" ? "<li class='upload-file success'><svg class='icon icon-sm' aria-hidden='true'><use href='../lib/svg/sprites.svg#it-file'></use></svg><p>". $row["uploadAttoNotarile"] ."</p></li>" : "";
                                                    ...
                                                    <div id="collapse-one" class="accordion-collapse collapse show" role="region" aria-labelledby="accordion-title-one">
                                                        <div class="accordion-body">
                                                            <ul class="link-list" data-element="page-index">
                                                                ...
                                                                <?php }else{ 
                                                                    if(CheckMyRatingService($_GET["aa_pratica_id"])){?>
                                                                    <li class="nav-item">
                    ...
                    <div class="col-12 col-xl-9 body-riepilogo">
                        <div class="row">
                            ...
                            if(!CheckRatingByCfService($_SESSION['CF'],'6')){
                                echo CallRatingLayout('aa_',$_GET["aa_pratica_id"],9);
                            }else{
                                echo ViewMyRatingStar('aa_',$_GET["aa_pratica_id"]);
                            }
                            ?>
                        </div>
