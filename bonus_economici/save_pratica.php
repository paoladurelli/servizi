...
$sqlCheck = "SELECT id FROM bonus_economici WHERE richiedenteCf = '". $_SESSION['CF']."' and id = " . $_POST['pratican'] ." and status_id = 0 and numeroPratica = ''";
$data['error'] = $sqlCheck;
...
if ($resultCheck->num_rows > 0) {
    ...
    $sqlNP = "SELECT NumeroPratica FROM bonus_economici WHERE status_id > 1 ORDER BY id DESC LIMIT 1";
    ...
    if ($resultNP->num_rows > 0) {
        while($rowNP = $resultNP->fetch_assoc()) {
            ...
            $length = 8;
            $NumeroPratica = "BE".str_pad($numberPraticaTmp2,$length,"0", STR_PAD_LEFT);
        }
    }else{
        $NumeroPratica = "BE00000001";
    }

    /* DATI ESTRAPOLATI DA DB - start */
    /* estrapolo i dati salvati con status tmp */
    $connessione = mysqli_connect($configDB['db_host'],$configDB['db_user'],$configDB['db_pass'],$configDB['db_name']);
    $sql = "SELECT * FROM `bonus_economici` WHERE id = " . $_POST['pratican'];
    ...
    if ($result->num_rows > 0) {
        while($row = $result->fetch_assoc()) {
            ...
            $NewuploadIsee = "";
            if(!empty($row["uploadIsee"])){
                $uploadIsee = $row["uploadIsee"];
                $NewuploadIsee = str_replace("_bozza_","_".$NumeroPratica."_",$uploadIsee);
                if (file_exists($upload_location.$uploadIsee)){
                    $renamed= rename($upload_location.$uploadIsee,$upload_location.$NewuploadIsee);
                }else{
                    $data['error'] = "The original file that you want to rename doesn't exist";
                }                    
            }
            ...
            $sqlDEL = "DELETE FROM attivita WHERE cf = '". $_SESSION['CF'] ."' AND servizio_id = 10 AND status_id = 1 AND pratica_id = ".$row['id_orig'];
            $connessioneDEL->query($sqlDEL);

            $sqlDEL = "DELETE FROM attivita WHERE cf = '". $_SESSION['CF'] ."' AND servizio_id = 10 AND status_id = 0 AND pratica_id = ".$_POST['pratican'];
            $connessioneDEL->query($sqlDEL);                

            $sqlDEL = "DELETE FROM bonus_economici WHERE id = ".$row['id_orig'];
            $connessioneDEL->query($sqlDEL);

            $sqlDEL = "DELETE FROM bonus_economici WHERE id = ".$_POST['pratican'];
            $connessioneDEL->query($sqlDEL);

            /* salvo nelle attitivà la creazione o modifica della bozza per bonus_economici */
                $sqlINS = "INSERT INTO attivita (cf,servizio_id,pratica_id,status_id,data_attivita) VALUES ('". $_SESSION['CF'] ."',10,".$new_id.",2,'".date('Y-m-d')."')";
                $connessioneINS->query($sqlINS);


            /* salvo nei messaggi che ho una bozza da completare per bonus_economici */
                $sqlINS = "INSERT INTO messaggi (CF_to,servizio_id,testo,data_msg) VALUES ('". $_SESSION['CF'] ."',10,'La tua richiesta di bonus economici è stata inviata.<br/>Il numero della pratica è: <b>".$NumeroPratica."</b>','".date('Y-m-d')."')";
                $connessioneINS->query($sqlINS);  

            /* preparo il pdf da allegare alla mail del comune */
                include '../lib/tcpdf/TCPDF-master/tcpdf.php';
                include '../lib/tcpdf/TCPDF-master/examples/be_pdf_comune.php'; 
            ...
            /* mando mail al comune - start */
                ...
                $phpmailer->Subject = 'Comune di '. $configData['nome_comune'] . ' - Richiesta di bonus economici  - '.$NumeroPratica.' - '. $_SESSION['CF'];
                ...
                $message = str_replace('%servizioselezionato%', 'richiesta di bonus economici', $message);
            ...
            /* mando mail all'utente - start */
                ...
                $phpmailer2->Subject = 'Comune di '. $configData['nome_comune'] . ' - Richiesta di bonus economici ';
    ...
    if($configDB['ProtocollazioneAttiva']){
        ...
        $sqlUPD = "UPDATE bonus_economici SET NumeroProtocollo='".$newProtocollo."' WHERE id = '".$new_id."'";
    ...
    if($configDB['AppIoAttiva']){
        SendToAppIo('bonus_economici',$NumeroPratica);
    }
