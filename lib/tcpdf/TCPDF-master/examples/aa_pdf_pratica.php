...
$numeroPratica = $_POST['aa_download_pdf_pratica'];

/* con l'id vado a richiamare i dati salvati */
    if(isset($_POST["aa_download_pdf_id"]) && $_POST["aa_download_pdf_id"]<>''){
        /* DATI ESTRAPOLATI DA DB - start */ 
        $connessione = mysqli_connect($configDB['db_host'],$configDB['db_user'],$configDB['db_pass'],$configDB['db_name']);
        $sql = "SELECT * FROM accesso_atti WHERE richiedenteCf = '". $_SESSION['CF']."' and id =" . $_POST["aa_download_pdf_id"];
...
class MYPDF extends TCPDF {
    ...
    public function Footer() {
            ...
            // Page number
            $this->Cell(0,10,'Pagina '.$this->getAliasNumPage().'/'.$this->getAliasNbPages() . ' - ServiziOnLine / Accesso agli atti',0,false,'C',0,'',0,false,'T','M');
    }
}
