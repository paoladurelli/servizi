...
class MYPDF extends TCPDF {
	...
	public function Footer() {
		...
		// Page number
		$this->Cell(0, 10, 'Pagina '.$this->getAliasNumPage().'/'.$this->getAliasNbPages() . ' - CRM Segnalazioni / Prenotazioni', 0, false, 'C', 0, '', 0, false, 'T', 'M');
	}
}
...
while ($row = $result_ticket_totale->fetch_array(MYSQLI_BOTH)) {
    ...
    if($row[4] == "Problema non bloccante"){
        $tipo_richiesta = "Problema Gravità Bassa";
    } else if ($row[4] == "Problema parzialmente bloccante"){
        $tipo_richiesta = "Problema Gravità Media";
    } else if($row[4] == "Problema bloccante"){
        $tipo_richiesta = "Problema Gravità Alta";
    } else {
        $tipo_richiesta = $row[4];
    }
}

if($tipo_richiesta == "Prenotazione Evento"){
    $label_data = "Data";
    $data_app_legg =  date("d-m-Y", strtotime($data_evento));
}  else {
...
$PRINT_VAR = 'Riepilogo Informazioni Prenotazione N. ' . $id . ' - ' . $data_apertura;
...
if($stato == 'IN ESSERE' ||$stato == 'RIAPERTO'){
    $statofixed = "IN ATTESA";
    $pdf->SetFillColor(224, 78, 92);
    $pdf->Cell(27,6,$statofixed,0,0, 'L',true);
    $pdf->Cell(13,6,'',0,0, 'L');
...
$pdf->Cell(60,6,$nomeecognome,0,0);
$pdf->Cell(0,6,$tipo_richiesta,0,1);
$pdf->Ln(5);
$pdf->SetFont('helvetica', '', 8);
$pdf->Cell(90,4,'Oggetto del ticket',0,0);
$pdf->SetFont('helvetica', '', 8);
$pdf->Cell(85,4,$label_data,0,1);
$pdf->SetFont('helvetica', '', 13);
$pdf->Cell(90,5,$oggetto,0,0);
$pdf->Cell(85,5,$data_app_legg,0,1);
$pdf->Ln(6);
$pdf->SetFont('helvetica', '', 8);
$pdf->Cell(0,6,'Numero di Telefono',0,1);
$pdf->SetFont('helvetica', '', 11);
$pdf->MultiCell(0,8,$numero_telefono,0,'L');
$pdf->Ln(6);
$pdf->SetFont('helvetica', '', 8);
...
//        $pdf->MultiCell(40,$maxnocells * 6,'RICH. PREVENTIVO','1','L',0,0);
    ...
    }else if($rowazioni[4] == 'IN ESSERE'){
        $pdf->Cell(40,$rowheight,'IN ATTESA',1,0,'L');
    }else{
...
ob_end_clean();
$pdf->Output('Documento-riassuntivo-appuntamento.pdf', 'D');
