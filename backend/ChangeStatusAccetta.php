...
if(!empty($_POST["ServizioId"]) && !empty($_POST["PraticaId"])) {
    ...
    $connessione = mysqli_connect($configDB['db_host'],$configDB['db_user'],$configDB['db_pass'],$configDB['db_name']);
        ...
        $sql = "UPDATE " . $table . " SET status_id = '4' WHERE id = '". $pratica_id."'";
        $result = $connessione->query($sql);
        if($result){
            $sqlA = "UPDATE attivita SET status_id = '4' WHERE servizio_id = '".$servizio_id."' AND pratica_id = '". $pratica_id."'";
            ...
            if($resultA){                
                ...
                $message = file_get_contents('template/mail/AccettataToUser.html');
                ...
                if($configDB['AppIoAttiva']){
                    $data['desc'] = SendToAppIoChangeStatus($table,$pratica_id,$cf_destinatario,'accettata');
                }
