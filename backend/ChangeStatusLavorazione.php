...
if(!empty($_POST["ServizioId"]) && !empty($_POST["PraticaId"])) {
    ...
    $connessione = mysqli_connect($configDB['db_host'],$configDB['db_user'],$configDB['db_pass'],$configDB['db_name']);
        ...
        $sql = "UPDATE " . $table . " SET status_id = '3' WHERE id = '". $pratica_id."'";
        $result = $connessione->query($sql);
        if($result){
            $sqlA = "UPDATE attivita SET status_id = '3' WHERE servizio_id = '".$servizio_id."' AND pratica_id = '". $pratica_id."'";
            ...
            if($resultA){
                ...
                $phpmailer->addAddress($mail_destinatario, $nome_destinatario);
                $phpmailer->Subject = 'Comune di '. $configData['nome_comune'] . ' - '.$servizioName.' | Nr Pratica: '. $NumeroPratica;
                $phpmailer->isHTML(true);

                $message = file_get_contents('template/mail/LavorazioneToUser.html');
                ...
                if($configDB['AppIoAttiva']){
                    $data['desc'] = SendToAppIoChangeStatus($table,$pratica_id,$cf_destinatario,'messa in lavorazione');
                }
